...
sub help {
  ...
  print "--mode (string, mandatory) - to tell POTION which main analysis mode. Possible values are: site or branch \n";
  ...
  print "\nThis is an example of how to tell POTION to create a configuration file named \"config_file.conf\" to analyze bacterial genomic data (genetic code 11), using 10 processors, with soft qualitative/ uantitative filtering criteria, analyzing only 1-1 orthologs, where sequence files are located at /home user/sequences/ , the homology relationship file is located at /home/user/homology_files/ xample.txt and output directory will be /home/user/POTION_results/ :\n  potion.pl --create_conf --stringency soft --outfile config_file.conf --CDS_dir_path /home/user/sequences/ --homology_file_path /home/user/homology_files/example.txt --max_processors 10 --output_dir /home/user/POTION_results/ --genetic_code 11 --homology_filter 1";
  print "\n\n";
}
...
sub read_config_files { #read config files in the form element = value #comment
  ...
  $parameters{specie_foreground} = read_config_file_line('specie_foreground', '', $fh_user_config);
  $parameters{minimum_taxa_background} = read_config_file_line('minimum_taxa_background', '', $fh_user_config);
  $parameters{minimum_taxa_foreground} = read_config_file_line('minimum_taxa_foreground', '', $fh_user_config);
  $parameters{maximum_taxa_background} = read_config_file_line('maximum_taxa_background', '', $fh_user_config);
  $parameters{maximum_taxa_foreground} = read_config_file_line('maximum_taxa_foreground', '', $fh_user_config);
  ...
  if ($parameters{mode} eq "site"){
    ...
    $parameters{PAML_models} = "mM0M1";
  } elsif($parameters{mode} eq "branchsite"){
    $parameters{PAML_models} = "mH0H1";
  }
  ...
  $parameters{validation_criteria} = read_config_file_line('sequence_validation_criteria', '', $fh_user_config);
  $parameters{fix_excess_nt} = read_config_file_line('fix_excess_nt', '', $fh_user_config);
  $parameters{absolute_min_sequence_size} = read_config_file_line('absolute_min_sequence_length', '', $fh_user_config);
  $parameters{absolute_max_sequence_size} = read_config_file_line('absolute_max_sequence_length', '', $fh_user_config);
  $parameters{sequence_size_average_metric} = read_config_file_line('sequence_length_average_metric', '', $fh_user_config);
  $parameters{relative_min_sequence_size} = read_config_file_line('relative_min_sequence_length', '', $fh_user_config);
  $parameters{relative_max_sequence_size} = read_config_file_line('relative_max_sequence_length', '', $fh_user_config);
  ...
  # -=-=-= PATH TO NEWICK UTILITIES PROGRAMS =-=-=-
  ...
  $parameters{nw_reroot} = read_config_file_line('nw_reroot', $parameters{potion_dir}, $fh_potion_config);
  ...
  #    print LOG ("POTION is setting min_gene_number_per_cluster to 4 because you choose phylogenetic_tree as $parameters{phylogenetic_tree}\n");
    die ("POTION aborting because min_gene_number_per_cluster is less to 4 and you phylogenetic_tree as $parameters{phylogenetic_tree}\n");
  }
  return \%parameters;
}
...
# function to set codon table name for codonphyml

sub codon_table_name{
 my $tb = $_[0];
 my %table_name = (
    '1' => 'STANDARD',
    '2' => 'TVMC',
    ...
    }else{ die("The value $tb for codon table is'n valid.")};
}
...
sub check_parameters { #check for all parameters, 
  ...
  if (-e $parameters->{project_dir_path}) {die("Directory ".$parameters->{project_dir_path}." already exist. Remove or rename it and try again.\n");}
  # -=-=-= INPUT FILES =-=-=-
  ...
  if($parameters->{mode} =~ m/branch/i){ # Parameter in branch or branch-site model
    if (!defined $parameters->{tree_file_path}) { die ("No path to user-defined tree file was specified in your project's configuration file, please, fill the parameter 'tree_file_path'.\n"); }
    if (defined $parameters->{tree_file_path}){
        if (!-e $parameters->{tree_file_path}) { die ("The tree file wasn't found in the specified path, please check if the path is correct: $parameters->{user_tree_file}\n"); }
        if (!-r $parameters->{tree_file_path}) { die ("You don't have permission to read the tree file, please redefine your permissions for this file.\n"); }
    }
  }
  ...
  if (!defined $parameters->{consense_path} or !exists($parameters->{consense_path})) { die ("No path to Consense was specified in potion_config at $config_path, please open this file and fill the parameter 'consense'.\n"); }
  ...
  if (defined $parameters->{phylogenetic_tree}){
    if ($parameters->{phylogenetic_tree} =~ /dnaml/i) {
    ...
    if ($parameters->{phylogenetic_tree} =~ /codonphyml/i) {
      if (!defined $parameters->{codonphyml_path}) { die ("No path to CodonphyML was specified in potion_config at $config_path, please open this file and fill the parameter 'codonphyml'.\n"); }
      if (!-s $parameters->{codonphyml_path}) { die ("The executable of CodonphyML wasn't found in the specified path, please check if the path is correct: $parameters->{codonphyml_path}\n"); }
    ...
    if ($parameters->{phylogenetic_tree} =~ /raxml/i ) {
  ...
  if(defined $parameters->{multiple_alignment}){
    if ($parameters->{multiple_alignment} =~ /mafft/i) {
    ...
    if ($parameters->{multiple_alignment} =~ /prank/i) {
    ...
    if ($parameters->{multiple_alignment} =~ /muscle/i) {
    ...
    if ($parameters->{multiple_alignment} =~ /clustalo/i) {
...
# NEWICK-UTILITIES
  ...
  if (!defined $parameters->{nw_reroot}) { die ("No path to nw_reroot was specified in potion_config at $config_path, please open this file and fill the parameter 'nw_reroot'.\n"); }
  if (!-s $parameters->{nw_reroot}) { die ("The executable of nw_reroot wasn't found in the specified path, please check if the path is correct: $parameters->{nw_reroot}\n"); }
  if (!-x $parameters->{nw_reroot}) { die ("You don't have permission to execute the nw_reroot file specified at potion_config, please check permissions or replace the file\n"); }

# -=-=-= MODULES =-=-=-

  if (!defined $parameters->{phylogenetic_tree} or !exists($parameters->{phylogenetic_tree})) { die ("No program specified for phylogenetic tree construction, please set the parameter 'phylogenetic_tree' in your project configuration file.\n"); }  
  ...
  if (!defined $parameters->{multiple_alignment} or !exists($parameters->{multiple_alignment})) { die ("No program specified for multiple alignment, please set the parameter 'multiple_alignment' in your project configuration file.\n"); }
  ...
  # -=-=-= PROJECT CONFIGURATION =-=-=-
  if (!defined $parameters->{codon_table} or !exists($parameters->{codon_table})) { die ("Codon table not specified, please set the parameter 'codon_table' in your project configuration file.\n"); }
  ...
  if (!defined $parameters->{recombination_qvalue} or !exists($parameters->{recombination_qvalue})) {die "Recombination q-value not setted (zero for no recombiation search). Please set recombination_qvalue element in configuration file\n";}

  if (!defined $parameters->{rec_minimum_confirmations} or !exists($parameters->{rec_minimum_confirmations})) {die "Minimum number of independent tests for recombination detection not setted (1, 2, 3 or N.A.). Please set recombination_qvalue element in configuration file\n";}

  if (!defined $parameters->{behavior_about_bad_clusters} or !exists($parameters->{behavior_about_bad_clusters})) {die "You must set the parameter \"behavior_about_bad_clusters\" with an integer ranging from 0 to 2, please check POTION documentation form more instructions about this parameter\n";}
  ...
  if (!defined $parameters->{homology_filter} or !exists($parameters->{homology_filter})) {die "You must set the parameter \"homology_filter\" with an integer ranging from 0 to 4, please check POTION documentation form more instructions about this parameter\n";}
...
#group identity parameters
  ...
  if (!defined $parameters->{max_group_identity} or !exists($parameters->{max_group_identity})) {
    $parameters->{max_group_identity} = 100;
    print "max_group_identity setted to 100, since it was not defined by user\n";
  }
  if (!defined $parameters->{group_identity_average_metric} or !exists($parameters->{group_identity_average_metric})) {
  ...
  if (!defined $parameters->{group_identity_comparison} or !exists($parameters->{group_identity_comparison})) {
...
#sequence identity parameters
  ...
  if (!defined $parameters->{min_sequence_identity} or !exists($parameters->{min_sequence_identity})) {
    $parameters->{min_sequence_identity} = 0;
    print "min_group_identity setted to 0, since it was not defined by user\n";
  }
  if (!defined $parameters->{max_sequence_identity} or !exists($parameters->{max_sequence_identity})) {
    $parameters->{max_sequence_identity} = 100;
    print "max_sequence_identity setted to 100, since it was not defined by user\n";
  }
  if (!defined $parameters->{sequence_identity_average_metric} or !exists($parameters->{sequence_identity_average_metric})) {
    $parameters->{sequence_identity_average_metric} = "mean";
    print "max_group_identity setted to 100, since it was not defined by user\n";
  }

  if (!defined $parameters->{sequence_identity_comparison} or !exists($parameters->{sequence_identity_comparison})) {
...
# 2- has either of the specified stop codons table
...
sub validate_sequence {
  ...
  # Condition 4 -  Presence of non-canonical nucleotides
...
sub parse_genome_files {
  ...
  while (my $file = readdir ($nt_files_dir)) {
    ...
    while (my $gene_info = $file_content->next_seq()) {
      ...
      if (exists($sequence_data{$accession_number})){
        die("The identifier $accession_number is duplicated in the file $file or appears in multiple files, please ensure that all sequence identifiers are unique even across different files");}
...
sub parse_homology_file {
  ...
  my %sequence_data = %{$sequences_ref}; #will store information about sequence, such as genome of origin, etc
                     ...
                     #in codeml branch and branch-site mode analysis
  ...
  if ($orthomcl_version eq "1.4") {
    while (my $line = <$fh_homology_file_path>) {
      ...
      # Step 3: remove clusters that don't have any of the reference genomes chosen by the user and separate the gene of the reference genome in the cluster.
      ...
      # Step 4: count the number of genes and species in the cluster
...
sub parse_pairs_file {
  ...
  while (my $line = <IN>) {
    ...
    if ($line =~ /^## AverageIdentity/) {
      ...
      if ($mean_identity < $parameters->{min_group_identity}) {
        ...
        print DUMMY ("STOP\nREMOVE_GROUP_FLAG\tQUALITY\t$$tmp_ortholog_group :mean group sequence identity ($mean_identity) smaller than cutoff:".$parameters->{min_group_identity}."\n");
        close DUMMY;
      }
      if ($mean_identity > $parameters->{max_group_identity}) {
        ...
        open (DUMMY,">>$$tmp_ortholog_group.group_status");
        print DUMMY ("STOP\nREMOVE_GROUP_FLAG\tQUALITY\t$$tmp_ortholog_group :mean group sequence identity ($mean_identity) greater than cutoff:".$parameters->{max_group_identity}."\n");
        close DUMMY;
      }
    }
      ...
      until ($line =~ /^\n$/) {
        ...
        if ($mean < $parameters->{min_sequence_identity}) { #removing sequence and groups if mean sequence identity with other sequences in groups are bellow user-defined cutoff
          ...
          if ((defined $parameters->{reference_genome_file})&&($tmpid2id_ref->{$id} eq $clusters_ref->{$$tmp_ortholog_group}{reference_gene})) {#means that the actual gene id is equal to the gene from the reference genome for this group and that user have selected a reference genome, therefore removing this group
            ...
            open (DUMMY,">>$$tmp_ortholog_group.group_status");
            print DUMMY ("STOP\nREMOVE_GROUP_FLAG\tQUALITY\t"."Group $$tmp_ortholog_group removed, mean identity of sequence $id ($mean) is smaller than cutoff (".$parameters->{min_sequence_identity}.") and $id is the reference gene for this group\n");
            close DUMMY;
          }
          if ($parameters->{behavior_about_bad_clusters} == 2) {#will remove group if users selected a scrict group cutoff and group contains a highly divergent sequence
            ...
            if (defined $clusters_ref->{$$tmp_ortholog_group}) {
              open (DUMMY,">>$$tmp_ortholog_group.group_status");
              print DUMMY ("STOP\nREMOVE_GROUP_FLAG\tQUALITY\t"."Group $$tmp_ortholog_group removed, mean identity of sequence $id ($mean) smaller than cutoff:".$parameters->{min_sequence_identity}." and behavior about bad clusters is on hard mode\n");
              close DUMMY;
            }
          }
        }
        if ($mean > $parameters->{max_sequence_identity}) { #removing sequence and groups if mean sequence identity with other sequences in groups are bellow user-defined cutoff
          ...
          if ((defined $parameters->{reference_genome_file})&&($tmpid2id_ref->{$id} eq $clusters_ref->{$$tmp_ortholog_group}{reference_gene})) {#means that the actual gene id is equal to the gene from the reference genome for this group and that user have selected a reference genome, therefore removing this group
            ...
            open (DUMMY,">>$$tmp_ortholog_group.group_status");
            print DUMMY ("STOP\nREMOVE_GROUP_FLAG\tQUALITY\t"."Group $$tmp_ortholog_group removed, mean identity of sequence $id ($mean) is greater than cutoff (".$parameters->{max_sequence_identity}.") and $id is the reference gene for this group\n");
            close DUMMY;
          }
          if ($parameters->{behavior_about_bad_clusters} == 2) {#will remove group if users selected a scrict group cutoff and group contains a highly divergent sequence
            print LOG "REMOVE_GROUP_FLAG\tQUALITY\t"."Group $$tmp_ortholog_group removed, mean identity of sequence $id ($mean) smaller than cutoff:".$parameters->{min_sequence_identity}." and behavior about bad clusters is on hard mode\n";
  ...
  if ($count < $parameters->{min_gene_number_per_cluster}) {
    ...
    if (defined $clusters_ref->{$$tmp_ortholog_group}) {
        open (DUMMY,">>$$tmp_ortholog_group.group_status");
        print DUMMY ("STOP\nREMOVE_GROUP_FLAG\tQUALITY\t"."Group $$tmp_ortholog_group removed, after removing divergent sequences it contains less sequences ($count) than the minimum number of genes per cluster:".$parameters->{min_gene_number_per_cluster}."\n");
        close DUMMY;
      return;
    }
  } else {
  ...
  while ($tries < $parameters->{tries} && !-s "$$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree") {
    ...
    try {
      if ($parameters->{phylogenetic_tree} =~ /proml/i) {
        ...
        move('outtree', "$$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree") || die ("Problem at $$ortholog_group (phylogenetic tree files): couldn't produce or manipulate the phylogenetic tree file.\n");  # renaming output file
      } elsif ($parameters->{phylogenetic_tree} =~ /codonphyml_aa|codonphyml_nt/i) {
        print LOG ("$parameters->{codonphyml_path} -i $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy -b $parameters->{bootstrap} -d $$seq_type -f optimize > $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree.log\n");
        my $stderr = capture_stderr{system("$parameters->{codonphyml_path} -i ./$$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy -b $parameters->{bootstrap} -d $$seq_type -f optimize > ./$$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree.log")};
        move("$$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy_codonphyml_tree.txt", "$$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree") || die ("Problem at $$ortholog_group (phylogenetic tree files): couldn't produce or manipulate the phylogenetic tree file.\n");  # renaming output file
      } elsif ($parameters->{phylogenetic_tree} =~ /codonphyml_co/i) {
        
        my $table_nm = codon_table_name($parameters->{codon_table});
      ...
      my $rand2 = int(rand(10000));
      until ($rand % 2) { $rand = int(rand(1000)); }
      until ($rand2 % 2) {$rand2 = int(rand(1000)); } 
      print LOG ("$parameters->{raxml_path} -s $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy -n $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree.log -m GTRGAMMA -p $rand -b $rand2 -N $parameters->{bootstrap}\n");
      my $stderr = capture_stderr{system("$parameters->{raxml_path} -s $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy -n $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree.log -m GTRGAMMA -p $rand -b $rand2 -N $parameters->{bootstrap} > /dev/null")};
      ...
      until ($rand % 2) { $rand = int(rand(1000)); }
      until ($rand2 % 2) { $rand2 = int(rand(1000)); }
      print LOG ("$parameters->{raxml_path} -s $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy -n $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree.log -m PROTCATmatrixName -p $rand -b $rand2 -N $$parameters->{bootstrap} &> /dev/null \n");
      my $stderr = capture_stderr{system("$parameters->{raxml_path} -s $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy -n $$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree.log -m PROTCATmatrixName -p $rand -b $rand2 -N $parameters->{bootstrap} > /dev/null")};
      move ("RAxML_bootstrap.$$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree.log", "$$ortholog_group.cluster.aa.fa.aln.$$seq_type.phy.trim.tree") || die ("Problem at $$ortholog_group (phylogenetic tree files): couldn't produce or manipulate the phylogenetic tree file.\n"); #renaming output file
...
sub calculate_dn_ds {
  ...
  if (-s "$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.model$model") { #for codeml
  ...
  if ($parameters->{mode} eq "branchsite") {
...
sub run_paml {
  ...
  while ($tries < $parameters->{tries} && !-s "$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.model$$model") {
    ...
    try { #capture_stderr uset to get ride of annoying STDERR messages
      ...
      move ("$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.model$$model.i", "$$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.model$$model") || die("Couldn't save paml result. Check the permitions in to intermediate_files dir.");
    } catch {
...
sub construct_table_header{
  ...
  foreach my $model(keys %models) {#creating first row types for each nested model
    if (defined $models{$model}) {
      ...
      if ($model eq "mM0M1") {
        $header = join ("\t", $header, "lnlmM0");
        $header = join ("\t", $header, "lnlmM1");
      }
    }
  }
...
sub parse_dn_ds_results {
  ...
  my @fh_for_blast = "";
  my $index = 0;
  my @possible_models = ("m12", "m78", "mH0H1", "m8a8", "mM0M1");
  ...
  open (my $fh_table, ">", "$parameters->{project_dir_path}/results/$parameters->{result_table}") || die("Cant open file parameters->{project_dir_path}/results/$parameters->{result_table}");
  open (my $fh_positive, ">", "$parameters->{project_dir_path}/results/$parameters->{result_positive}")|| die("Can't open file $parameters->{project_dir_path}/results/$parameters->{result_positive}");
  #  open (my $fh_uncertain, ">", "$parameters->{project_dir_path}/results/$parameters->{result_uncertain}");
  open (my $fh_positive_interleaved, ">", "$parameters->{project_dir_path}/results/$parameters->{result_positive}.interleaved") || die("Can't open file $parameters->{project_dir_path}/results/$parameters->{result_positive}.interleaved");
  ...
  if (defined $nested_models_to_be_tested{"mH0H1"}) {
    open ($fh_pos_mH0H1_sequences, ">", "$parameters->{project_dir_path}/results/$parameters->{result_positive}.positives_mH0H1.fasta")||die("Cant create file $parameters->{project_dir_path}/results/$parameters->{result_positive}.positives_mH0H1.fasta");
  ...
  foreach my $ortholog_group (keys %{$clusters_ref}){
    ...
    if ($parameters->{mode} eq "branch"){
      if (-e "$ortholog_dir$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.modelM0") {
        open (my $fh_paml, "<", "$ortholog_dir$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.modelM0");
        $data{$ortholog_group}{M0} = find_ln_value($fh_paml);
        close($fh_paml);
      }

      if (-e "$ortholog_dir$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.modelM1") {
        open (my $fh_paml, "<", "$ortholog_dir$ortholog_group.cluster.aa.fa.aln.nt.phy.trim.paml.modelM1");
        $data{$ortholog_group}{M1} = find_ln_value($fh_paml);
        ($data{$ortholog_group}{gene_id}, $data{$ortholog_group}{temp_id}, $data{$ortholog_group}{selected_aminoacids_M1}) = find_BEB_info($fh_paml, $tmp_id2id_ref, \$ortholog_dir);
        close($fh_paml);
      }  
    }
    ...
    if (!defined $data{$ortholog_group}{M0} || !defined $data{$ortholog_group}{M1}) {
      $data{$ortholog_group}{no_value_M0M1} = 1;
    }
  ...
  #LEO - para calcular p value para H0 H1 
  my %p_value_H0H1;
  my $q_values_H0H1;
  my @no_value_H0H1;
  if ($parameters->{PAML_models} =~ /h0h1/i) {
    foreach my $ortholog_group (keys %data) {
      if ($data{$ortholog_group}{no_value_H0H1}) {
        ...
        $p_value_H0H1{$ortholog_group} = Statistics::Distributions::chisqrprob(1, $data{$ortholog_group}{likelihood_ratio_H0H1});
      }
    }
  ...
  my $q_values_M0M1;
  my @no_value_M0M1;
  if ($parameters->{PAML_models} =~ /M0M1/i) {
    foreach my $ortholog_group (keys %data) {
      if ($data{$ortholog_group}{no_value_M0M1}) {
        push(@no_value_M0M1, $ortholog_group);
      } else {
        $data{$ortholog_group}{likelihood_ratio_M0M1} = 2*($data{$ortholog_group}{M1} - $data{$ortholog_group}{M0});
        $p_value_M0M1{$ortholog_group} = Statistics::Distributions::chisqrprob(1, $data{$ortholog_group}{likelihood_ratio_M0M1});
      }
    }
    if (scalar keys %p_value_M0M1) {
      $q_values_M0M1 = eval 'qvalue(\%p_value_M0M1)';
      if($@) { $q_values_M0M1 = BY(\%p_value_M0M1); }
    }
  }
  ...
  elsif ($parameters->{PAML_models} =~ /^mh0h1/i && defined $q_values_H0H1) {
    foreach my $ortholog_group (sort {$q_values_H0H1->{$a} <=> $q_values_H0H1->{$b}} keys %{$q_values_H0H1}) {
      push(@sorted_values, $ortholog_group);
    }
  }
  elsif ($parameters->{PAML_models} =~ /^mM0M1/i && defined $q_values_M0M1) {
    foreach my $ortholog_group (sort {$q_values_M0M1->{$a} <=> $q_values_M0M1->{$b}} keys %{$q_values_M0M1}) {
      push(@sorted_values, $ortholog_group);
    }
  }
  ...
  if ($parameters->{mode} eq "site") {
    ...
    foreach my $ortholog_group (@sorted_values) {
      print $fh_table ($ortholog_group, "\t");
      if (!defined $data{$ortholog_group}{M0} || !defined $data{$ortholog_group}{M1} || !defined $q_values_M0M1->{$ortholog_group}) {
        print $fh_table ('-', "\t");
      ...
      if (defined $data{$ortholog_group}{M0})  { print $fh_table (sprintf("%.6f", $data{$ortholog_group}{M0}), "\t"); }
      else { print $fh_table ("-\t"); }
      if (defined $data{$ortholog_group}{M1}) { print $fh_table (sprintf("%.6f", $data{$ortholog_group}{M1}), "\t"); }
      else { print $fh_table ("-\t"); }

      if (defined $data{$ortholog_group}{likelihood_ratio_M0M1}) {
        print $fh_table (sprintf("%.6f", $data{$ortholog_group}{likelihood_ratio_M0M1}), "\t");
      } else {
        print $fh_table ("-\t");
      }

      if (defined $p_value_M0M1{$ortholog_group}) {
        print $fh_table (sprintf("%.6f", $p_value_M0M1{$ortholog_group}), "\t");
      } else {
        print $fh_table ("-\t");
      }

      if (defined $q_values_M0M1->{$ortholog_group}) {
        print $fh_table (sprintf("%.6f", $q_values_M0M1->{$ortholog_group}), "\n");
      } else {
        print $fh_table ("-\n");
      }
    }
...
sub find_ln_value {
  ...
  while (my $line = <$fh_paml>) {
    ...
    if ($line =~ /^lnL/){
      ...
      print LOG ("PARSED lnL value: $test\n");
      return $test;
    }
  }
  return;
...
sub manage_task_allocation {
  my ($pid_to_ortholog_group, $n_process, $phi_pvalues, $nss_pvalues, $maxchi2_pvalues, $parameters, $id_rec_to_process, $f_tree_to_process, $model8_to_process, $model8a_to_process, $model7_to_process, $model2_to_process, $model1_to_process, $modelH1_to_process, $modelH0_to_process, $modelM1_to_process, $modelM0_to_process, $seq_type) = @_;
  ...
  if (exists $pid_to_ortholog_group->{$completed_pid}) {
   ...
   # variable added just for human readability, terrible without it
    ...
    if ((!exists $phi_pvalues->{$ortholog_group})&&($parameters->{recombination_qvalue} !~ /N.A./i)) { # if there's no phi_pvalue for a group, get it
      ($phi_pvalues->{$ortholog_group}, $nss_pvalues->{$ortholog_group}, $maxchi2_pvalues->{$ortholog_group}) = read_phi($id_rec_file);

      if (($$n_process == 1 && !@{$id_rec_to_process})&&($parameters->{recombination_qvalue} !~ /N.A./i)) { # then, all groups had their recombination pvalue measured; proceeding with multiple hypothesis correction
    ...
    # If a f_tree has finished instead
        ...
        push(@{$modelH1_to_process}, "$ortholog_group;modeH1") if ($parameters->{PAML_models} =~ /mH0H1/i);
        push(@{$modelH0_to_process}, "$ortholog_group;modeH0") if ($parameters->{PAML_models} =~ /mH0H1/i);
        push(@{$modelM1_to_process}, "$ortholog_group;modeM1") if ($parameters->{PAML_models} =~ /mM0M1/i);
        push(@{$modelM0_to_process}, "$ortholog_group;modeM0") if ($parameters->{PAML_models} =~ /mM0M1/i);
...
sub create_tree_files_branch_mode {
  ...
  #    print("11\t*key:$key*\t*group:$sequence_data->{$key}{group}*\t*group2:$$ortholog_group*\t*status:$sequence_data->{$key}{status}*\n");
    if ((defined $sequence_data->{$key}{group})&&($sequence_data->{$key}{group} eq "$$ortholog_group")&&($sequence_data->{$key}{status} eq "OK")) { #to print only
       if (($parameters->{'maximum_taxa_foreground'} == 1) && ($parameters->{'specie_foreground'} eq $sequence_data->{$key}{genome})) {
        print OUTDIC "$sequence_data->{$key}{genome} $id2tmp_id->{$key}"."#1"."\n";
      } else {
  ...
  #  my $stderr = capture_stderr {system ("$parameters->{nw_rename} /home/thieres/project/genetime/results/tree.nwk $$ortholog_group.id2genome > $$ortholog_group.dummy_tree")};
  ...
  #
  print LOG ("$parameters->{nw_rename} $parameters->{tree_file_path}
  $$ortholog_group.id2genome > $$ortholog_group.dummy_tree\n");
  if ($stderr) {
    open (OUTERR,">>$$ortholog_group.group_status");
...
sub trim_tree_file {
  ...
  my $stderr = capture_stderr {system ("$parameters->{nw_labels} $$ortholog_group.dummy_tree > $$ortholog_group.tree_labels")};
  ...
  #removing genes filtered out in previous steps
  my %ok_ids;
  #  my $i2 = 0;
  my $root_dir = "$parameters->{project_dir_path}/intermediate_files/".$$ortholog_group.'/';
  my $ok_genes_path = $root_dir."id_names";
  open(IN, $ok_genes_path) ||
    die("Could not open $ok_genes_path, please check!\n");
  while (my $line = <IN>) {
    chomp $line;
    my @aux = split(/ => /, $line);
    $ok_ids{$aux[0]} = 1;
  ...
  foreach my $name (@tmp_names) {
    if (defined $ok_ids{$name}) {
     
    } else {
      if (($name =~ m/#1/) || ($name =~ m/\$1/)) {
      } #indicates foreground branch, should not be removed
      else {
        $removed_names[$i] = $name;
        $i++;
      }
  ...
  #get foreground species before pruning tree
  if ($parameters->{maximum_taxa_foreground} > 1){ 
    if ($label_type == 1) {
      $stderr = capture_stderr {system ("$parameters->{nw_clade} $$ortholog_group.dummy_tree '#1' > $$ortholog_group.tmp_foreground_branch_not_filtered")};
    }
    if ($label_type == 2) {
      $stderr = capture_stderr {system ("$parameters->{nw_clade} $$ortholog_group.dummy_tree '\$1' > $$ortholog_group.tmp_foreground_branch_not_filtered")};
    }
    $stderr = capture_stderr {system ("$parameters->{nw_labels} $$ortholog_group.tmp_foreground_branch_not_filtered > $$ortholog_group.foreground_labels_not_filtered")};
  }
  #removing species names
  if (@removed_names) {
    my $stderr = capture_stderr {system ("$parameters->{nw_prune} $$ortholog_group.dummy_tree @removed_names > $$ortholog_group.new_dummy_tree")};
    print LOG ("$parameters->{nw_prune} $$ortholog_group.dummy_tree @removed_names > $$ortholog_group.new_dummy_tree\n");
    move("$$ortholog_group.dummy_tree", "$$ortholog_group.old_dummy_tree");
    move("$$ortholog_group.new_dummy_tree", "$$ortholog_group.dummy_tree");
  }

  #$stderr = capture_stderr {system ("$parameters->{nw_clade} $$ortholog_group.dummy_tree '#1' > $$ortholog_group.tmp_foreground_branch")};
  #$stderr = capture_stderr {system ("$parameters->{nw_labels} $$ortholog_group.tmp_foreground_branch > $$ortholog_group.foreground_branch")};
  if ($parameters->{maximum_taxa_foreground} !=1 ){
    my $dummy_tree = Bio::TreeIO->new('-format' => 'newick', '-file' => "$$ortholog_group.dummy_tree")->next_tree();
    my $check_mark = 'Not_ok';
    for my $node($dummy_tree->get_nodes()){
      my $marked_node = $node->id;
      if ($marked_node){
        if (($marked_node =~ m/#1/) || ($marked_node =~ m/\$1/)){
          $check_mark = 'Ok';
        }
      }  
    }
    if ($check_mark eq 'Not_ok'){
      my @labels_not_filtered;
      my @labels_filtered;
      my $n = 0;
      open (IN, "<$$ortholog_group.foreground_labels_not_filtered") or die;
      while (my $line = <IN>){
        chomp $line;
        if (($line eq '#1') || ($line eq "\$1")){
          next;
        }
        $labels_not_filtered[$n] = $line;
        $n++
      }
      ...
      foreach my $labels(@labels_not_filtered){
        if (grep(/^$labels/, @removed_names)){next}
          $labels_filtered[$n] = $labels;
          $n++;
      }

      my @foreground_node;
      for my $labels(@labels_filtered){
        my @nodes = $dummy_tree->find_node(-id => $labels);
        push (@foreground_node, @nodes);
      }
      my $lca = (@foreground_node==1)?$foreground_node[0]:$dummy_tree->get_lca(-nodes => \@foreground_node);
      if ($label_type == 1){
        $lca->id($lca->id."#1");
      }
      if ($label_type == 2){
      ...
      Bio::TreeIO->new ('-format' => 'newick' , '-file' => ">$$ortholog_group.dummy_tree")->write_tree($dummy_tree); 
    } 
    ...
    if (($for_count < $parameters->{minimum_taxa_foreground})||($for_count > $parameters->{maximum_taxa_foreground})) {
      ...
      if (defined $clusters->{$$ortholog_group}) {
        ...
        print DUMMY ("STOP\nREMOVE_GROUP_FLAG\tPHYLOGENETIC\t"."Group $$ortholog_group removed: the number of foreground taxa ($for_count) is outside user-defined cutoffs ($parameters->{minimum_taxa_foreground} - $parameters->{maximum_taxa_foreground})\n");
        close DUMMY;
      }
    }
    if (($back_count < $parameters->{minimum_taxa_background})||($back_count > $parameters->{maximum_taxa_background})) {
      if (defined $clusters->{$$ortholog_group}) {
        ...
        open (DUMMY,">>$$ortholog_group.group_status");
        print DUMMY ("STOP\nREMOVE_GROUP_FLAG\tPHYLOGENETIC\t"."Group $$ortholog_group removed: the number of background taxa ($back_count) is outside user-defined cutoffs ($parameters->{minimum_taxa_background} - $parameters->{maximum_taxa_background})\n");
        close DUMMY;
    ...
    my $status = "NOT_OK";
    my @labels;
    my $stderr = capture_stderr {system ("$parameters->{nw_labels} $$ortholog_group.dummy_tree > $$ortholog_group.labels_filtered")};
    open (IN, "<$$ortholog_group.labels_filtered") or die;
    while (my $line = <IN>){
      chomp $line;
    ...
    if ($status eq "NOT_OK"){
      print "Group $$ortholog_group removed: No species marked for foreground branch})\n";
      if (defined $clusters->{$$ortholog_group}) {
        print LOG "REMOVE_GROUP_FLAG\tPHYLOGENETIC\t"."Group $$ortholog_group removed: No species marked for the foreground branch\n";
        open (DUMMY,">>$$ortholog_group.group_status");
        print DUMMY ("STOP\nREMOVE_GROUP_FLAG\tPHYLOGENETIC\t"."Group $$ortholog_group removed: No species marked for the foregound branch\n");
        close DUMMY;
      }
    }
    #print Dumper (@labels);
