...
my $tree_file_path = ""; #path to phylogenetic tree used in branch-site model
...
"tree_file_path=s"=> \$tree_file_path,
);
...
if (!-e $parameters_ref->{project_dir_path}) { mkdir ($parameters_ref->{project_dir_path}) || die ("Couldn't create the directory specified as '$parameters_ref->{project_dir_path}', check if you are trying to create a subdirectory in a non-existent directory or if you don't have permission to create a directory there.\n"); }
  ...
  print("Directory already exists, trying to resume a previous run!\n");
}
...
my @modelM0_to_process;
my @modelM1_to_process;
...
foreach my $ortholog_group (keys %{$clusters_ref}) {
  push (@id_rec_to_process, "$ortholog_group;id_rec"); 
  push (@f_tree_to_process, "$ortholog_group;f_tree"); 
}

while (@id_rec_to_process > 0 || @f_tree_to_process > 0 || @model8_to_process > 0 || @model8a_to_process > 0 ||  @model7_to_process > 0 || @model2_to_process > 0 || @model1_to_process > 0 || @modelH1_to_process > 0 || @modelH0_to_process > 0 || @modelM0_to_process || @modelM1_to_process || $n_process > 0) { #defining which task will start when a free processor is available
  my $ortholog_group;
  if ($n_process == $parameters_ref->{max_processors}) { #if a processor is available, start a new task
    manage_task_allocation(\%pid_to_ortholog_group, \$n_process, \%phi_pvalues, \%nss_pvalues, \%maxchi2_pvalues, $parameters_ref, \@id_rec_to_process, \@f_tree_to_process, \@model8_to_process, \@model8a_to_process, \@model7_to_process, \@model2_to_process, \@model1_to_process, \@modelH1_to_process, \@modelH0_to_process,\@modelM1_to_process, \@modelM0_to_process, \$seq_type);
  }
  ...
  elsif (@modelM1_to_process) { $ortholog_group = shift(@modelM1_to_process); }
  elsif (@modelM0_to_process) { $ortholog_group = shift(@modelM0_to_process); }
  else {
    manage_task_allocation(\%pid_to_ortholog_group, \$n_process, \%phi_pvalues, \%nss_pvalues, \%maxchi2_pvalues, $parameters_ref, \@id_rec_to_process, \@f_tree_to_process, \@model8_to_process, \@model8a_to_process, \@model7_to_process, \@model2_to_process, \@model1_to_process, \@modelH1_to_process, \@modelH0_to_process, \@modelM1_to_process, \@modelM0_to_process, \$seq_type);
    next;
  }
  my $task = substr($ortholog_group, -6, 6); # either id_rec, f_tree, model8, mode8a, model7, model2, model1, modeH0, modeH1, modeM0 or modeM1
  ...
  if (!$pid) {
    ...
    if ($task eq 'id_rec') {
      ...
      if ($parameters_ref->{mode} eq "site") { 
      ...
      } elsif ($parameters_ref->{mode} eq "branch" || $parameters_ref->{mode} eq "branchsite") {
        ...
        print_task_time(\$ortholog_dir, \$task_time, 'time_f_tree');
          ...
          die("site, branch or branch-site models for codeml must be configured.\n");
      }
    } elsif ($task eq 'model8' || $task eq "mode8a" || $task eq 'model7' || $task eq 'model2' || $task eq 'model1'|| $task eq "modeH1" || $task eq "modeH0" || $task eq "modeM1" || $task eq "modeM0") {
      my $task_time = time();
      ...
      elsif ($task eq 'modeM1') {calculate_dn_ds($parameters_ref, \$potion_path, \$seq_type, \$ortholog_group, 'M1'); print_task_time(\$ortholog_dir, \$task_time, 'time_model_M1'); }
      elsif ($task eq 'modeM0') {calculate_dn_ds($parameters_ref, \$potion_path, \$seq_type, \$ortholog_group, 'M0'); print_task_time(\$ortholog_dir, \$task_time, 'time_model_M0'); }
    }
    exit(0);
  }
